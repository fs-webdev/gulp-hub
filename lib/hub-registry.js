...
HubRegistry.prototype.init = function (gulp) {
	...
	var masterGulp = new gulp.Gulp();

	// emit all events back to the main gulp instance
	masterGulp.emit = function(eventName) {
		gulp.emit.apply(gulp, arguments);

		// due to the masterGulp task being passed and used in the parent tasks,
		// errors do not end the process and give an exit code of 0.
		// we need to force the exit code to stop a build from deploying with errors
		if (eventName === 'error') {
			process.exit(1);
		}
 	}
	...
	_.each(this._registry, function(subregistry, subfileName) {
		...
		_.each(subtasks, function(subtask, name) {
			...
			// pass the masterGulp instance to allow the task to use it to call all
			// subtasks of the same name from different gulpfiles (such as build:js)
			// @see https://github.com/frankwallis/gulp-hub/issues/29
			sandboxedGulp.task(subname, function taskWrapper() {
				var args = Array.prototype.slice.call(arguments);
				return subtask.apply(this, args.concat(masterGulp));
			});
	...
	_.each(master, function(subtasks, name) {
		if (subtasks.length == 1)
			masterGulp.task(name, subtasks[0]);
		else
			masterGulp.task(name, gulp.series(subtasks));
	})

	this._tasks = masterGulp.registry().tasks();
};
